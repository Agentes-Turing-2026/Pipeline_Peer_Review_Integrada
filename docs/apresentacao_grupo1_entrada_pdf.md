...
|---|---|
| Validar problemas básicos antes da extração (inexistente, formato, protegido, corrompido, ilegível). | `classificar_arquivo_pdf` em [`src/validacao_entrada.py`](../src/validacao_entrada.py): existência, é-arquivo, extensão `.pdf`, bytes > 0, cabeçalho `%PDF`, `/Encrypt`. PDF protegido/corrompido que só o parser detecta é capturado por `registrar_falha_extracao` (evento estruturado, não traceback). |
...
| Conectar ao trace preservando `run_id` e as categorias. | Eventos via `emitir_evento` (mesmo arquivo/`run_id`/trace); categorias `passou_de_primeira`, `alerta_entrada`, `falhou_recuperavel`, `corrigido`, `passou_apos_correcao`, `bloqueado`. |
...
| Verificar código legacy sem uso; remover/arquivar em commit separado. | `legacy_adapter.py` não era importado por ninguém → **arquivado** em `src/archive/` (com README explicando) em commit próprio, após os testes passarem. |
...
2. **Alerta** — texto curto / aviso do extrator → `alerta_entrada` (segue, revisão humana).
...
- **Reuso do sistema de eventos.** Zero logs paralelos: mesma trilha,
  `run_id` e trace já existentes. Categoria `alerta_entrada` foi acrescentada ao
