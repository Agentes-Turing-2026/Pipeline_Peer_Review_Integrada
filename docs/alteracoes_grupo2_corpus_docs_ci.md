...
Branch `grupo-2-corpus-publico-docs-e-ci`, criada a partir de `dev`, onde o
pull request (#20) foi aberto. O merge acabou sendo feito **direto na `main`**,
por orientação do supervisor: os PRs #21 e #22 já tinham entrado na `main` sem
passar pela `dev`, e a `main` estava sem CI — este merge é o que leva
`ci.yml`, `pyproject.toml` e `poetry.lock` para lá. Ver §9.2.
...
| Mypy no repositório inteiro | 38 erros | **25** |
...
> [!IMPORTANT]
> **Os apontamentos restantes estão integralmente em arquivos de outros
...
### 9.1. Estado no momento do merge na `main` (18/08/2026)

Entre a abertura deste PR e o merge, entraram na `main` o PR #21 (Grupo 1) e o
PR #22 (Grupo 3). Números remedidos sobre a árvore já mergeada, com as versões
exatas do `poetry.lock` (`ruff 0.16.2`, `mypy 2.3.0`) e a régua deste
`pyproject.toml`:

| Ferramenta | Na abertura do PR | Após #21 e #22 | Nos arquivos do Grupo 2 |
|---|---|---|---|
| Ruff | 8 | **8** | **0** |
| Mypy | 32 | **25** | **0** |
| Bandit (MEDIUM+) | 0 | **0** (7 LOW) | **0** |
| Pytest (local) | 280 ✅ | **281 ✅** / 9 pulados | — |
...
| **Grupo 1** — pasta `demos/` | 3 | 9 | `demos/demo_validacao.py` (3+7), `demos/demo_persistencia.py` (2) |
| **Grupo 2** | **0** | **0** | — |

> [!NOTE]
> **Os 21 do núcleo nunca tiveram dono.** O gerente do projeto escopou cada
> grupo por área: o Grupo 1 para "os arquivos de validação, retry e
> persistência que vocês modificaram", o Grupo 2 para "os arquivos de métricas
> e benchmark", o Grupo 3 para as referências ao `sabia-3`. Os três cumpriram o
> escopo recebido — o PR #21 zerou `validacao_retry.py`, `validacao_entrada.py`
> e `eventos_validacao.py`, e o PR #22 fechou o `sabia-3` residual. O que
> sobrou em `pipeline.py`, `cross_review.py` e `editor_agent.py` não entrou no
> escopo de ninguém, e é o maior bloco: 21 dos 33.
...
Os 25 do Mypy são manuais, porém dois terços estão em `pipeline.py` e repetem
dois padrões: `Any | None` atribuído a campo tipado (`assignment`) e atributo
acessado sem verificação de `None` (`union-attr`).

### 9.2. O CI chega na `main` com este merge

Até aqui, `.github/workflows/ci.yml`, `pyproject.toml` e `poetry.lock`
existiam **apenas** na `dev` (commit do supervisor) e nesta branch. A `main`
não tinha nenhum dos três, e por isso os PRs #21 e #22 — que foram mergeados
direto nela — **não passaram por Ruff, Mypy, Bandit ou Pytest**.

Este merge leva o CI para a `main`. A consequência imediata e esperada é que a
primeira execução do job na `main` fique **vermelha em Ruff e Mypy**, com os 33
apontamentos da tabela acima. Nenhum deles é de arquivo do Grupo 2. Bandit e
Pytest passam.

### Pendências

- [ ] **Responsável pelo núcleo compartilhado:** 5 Ruff + 16 Mypy em
      `pipeline.py`, `cross_review.py` e `editor_agent.py` — o bloco que nunca
      foi atribuído
- [ ] **Grupo 1:** 3 Ruff + 9 Mypy na pasta `demos/`
- [ ] **Marcelo:** validação das alterações em `ci.yml` e `pyproject.toml`
- [ ] **Nova execução do benchmark, com o corpus completo em modo `api`** —
      depende das duas primeiras
- [x] ~~**Grupo 3:** `.env.example` linha 34 (`LLM_FALLBACK_MODEL=sabia-3`)~~ —
      resolvido no PR #22
