...
def run_demo(
    ...
    """
    # Retomada: recupera pdf_path/mode/cross_review da execução original quando
    # não informados explicitamente nesta chamada — sem isso, retomar com um
    ...
    # dela — para não poluir `fases_concluidas()`) grava pdf_path/mode/
    # cross_review só na primeira execução, para uma retomada futura
    # recuperá-los.
    ...
    def _run_and_save() -> FinalReport:
        ...
        if coletor is not None and gerar_resumo is not None:
            resumo = gerar_resumo(
                coletor.eventos, run_id=coletor.run_id,
                duracao_total_s=coletor.duracao_execucao_s, precos=precos_execucao,
            )
