...
def test_retry_alerta_segue(tmp_path):
    ...
    assert categorias[-1] == "alerta_entrada"
...
# ---------------------------------------------------------------------------
# Falha DURANTE a extração (PDF protegido/corrompido detectado pelo parser)
# ---------------------------------------------------------------------------

def test_registrar_falha_extracao_emite_bloqueado():
    exc = registrar_falha_extracao(
        "artigo.pdf", RuntimeError("PDF protegido por senha"), run_id="run-fx",
    )
    assert isinstance(exc, EntradaInvalidaError)
    eventos = ev.ler_eventos(run_id="run-fx")
    assert eventos[-1]["categoria"] == "bloqueado"
    assert "protegido" in eventos[-1]["erro"]
    assert eventos[-1]["requer_revisao_humana"] is True


def test_extract_pdf_input_registra_falha_de_extracao(tmp_path):
    """Se o extrator do Grupo 3 levanta, vira EntradaInvalidaError + evento estruturado."""
    import pipeline

    class _ExtratorQuebra:
        name = "quebra"
        version = "0.0"
    ...
    with pytest.raises(EntradaInvalidaError, match="protegido"):
        pipeline.extract_pdf_input(
            _pdf_valido(tmp_path), extractor=_ExtratorQuebra(), tracer=None, run_id="run-px",
        )
    eventos = ev.ler_eventos(run_id="run-px")
    assert eventos[-1]["categoria"] == "bloqueado"
